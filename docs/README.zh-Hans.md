...
- 🌪️ 导出主页时间线和列表时间线中的推文
- 🔍 导出搜索结果
- ✉️ 导出私信（目前不可用，详见 [#116](https://github.com/prinsss/twitter-web-exporter/issues/116)）
- 📦 以原始尺寸批量下载推文中的图片和视频
- 🚀 无需开发者账号或 API 密钥
...
设置面板中还提供了一个“手动批量抓取”辅助工具，可用于书签和当前账号推文的批量抓取。它会以较保守的节奏自动打开目标页面并滚动，但仍然依赖 Twitter/X 网页端自己加载数据。

设置面板现在还提供了一个“浏览器迁移”流程。你可以导出一个单独的迁移包，其中同时包含本地数据库和完整配置，再在另一个浏览器中导入以恢复相同环境。请注意，导入迁移包会覆盖当前浏览器里的本地数据库和设置，而且文件中可能包含同步凭证或访问令牌等敏感信息，请妥善保管。
...
导出过程仍然主要是“网页辅助式”的。内置的“手动批量抓取”可以为书签和当前账号推文自动切页并滚动，但它依然依赖 Twitter/X 网页端自己加载数据。脚本本身不会向 Twitter API 发送任何请求。
...
**问：导出过程可以自动化吗？** <br>
答：可以部分自动化。内置的“手动批量抓取”可以辅助自动滚动抓取书签和当前账号推文。对于其他流程，你仍然需要手动浏览并加载数据，或者配合第三方工具使用。社区方案参见 [#120](https://github.com/prinsss/twitter-web-exporter/issues/120)。

**问：可以把数据和设置迁移到另一个浏览器吗？** <br>
答：可以。在设置面板里使用迁移包的导出和导入功能即可。迁移文件会同时包含本地数据库和完整设置快照。导入时会覆盖当前浏览器中的本地数据库和设置。
